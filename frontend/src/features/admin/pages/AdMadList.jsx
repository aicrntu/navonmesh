...
import { getAllAdmad } from "../../../api/admad.api";
import { useAdminList } from "../../../hooks/useAdminList";
import { exportToExcel } from "../../../utils/excelExport";
import { Download, Search, ChevronLeft, ChevronRight } from "lucide-react";

export default function AdMadList() {
  const BACKEND_URL = import.meta.env.VITE_BACKEND_URL;
  const [data, setData] = useState([]);

  useEffect(() => {
    getAllAdmad().then((res) => {
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <h1 className="text-2xl font-bold">Ad-Mad Submissions</h1>
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <button
            onClick={() => exportToExcel(data, "AdMad_Submissions")}
          ...
          <tbody>
            {paginatedData.map((item, index) => (
              ...
              >
                ...
                <td className="px-3 py-3 font-medium text-gray-900 border-x">
                  {item.collegeName}
                </td>
                <td className="px-3 py-3">{item.numberOfTeamMember}</td>
